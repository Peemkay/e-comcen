...
class SecurityProvider extends ChangeNotifier {
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      _sessionTimeoutSubscription =
          _securityService.onSessionTimeout.listen((timeout) {
        if (timeout) {
  ...
  Future<bool> authenticateUser({bool biometricOnly = false}) async {
    try {
      final result =
          await _securityService.authenticateUser(biometricOnly: biometricOnly);
  ...
  void sessionTimeout() {
    ...
    // No need to log the event here as the security service
    // will automatically log it when the session times out
  }
