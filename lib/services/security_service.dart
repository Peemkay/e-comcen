...
// import 'package:flutter_jailbreak_detection/flutter_jailbreak_detection.dart'; // Temporarily commented out
...
class SecurityService {
  ...
  Future<bool> _checkDeviceSecurity() async {
    try {
      ...
      // Temporarily disabled jailbreak detection
      // Commented out due to plugin issues
      // final isJailbroken = await FlutterJailbreakDetection.jailbroken;
      // final isDeveloperMode = await FlutterJailbreakDetection.developerMode;

      // Jailbreak detection is disabled, so we'll assume the device is secure
      _logSecurityEvent(
        SecurityEventType.securityCheckPassed,
        'Jailbreak detection is disabled, assuming device is secure',
      );
  ...
  Future<void> _addAuthorizedDevice(String deviceId) async {
    ...
    await _secureStorage.write(
        key: 'authorized_devices', value: devices.join(','));
  ...
  final StreamController<bool> _sessionTimeoutController =
  ...
  Future<void> _saveSecurityLogs() async {
    try {
      ...
      final logsJson =
          jsonEncode(_securityLogs.map((log) => log.toJson()).toList());
  ...
  Future<void> _loadSecurityLogs() async {
    try {
      ...
      if (await file.exists()) {
        ...
        _securityLogs =
            logsList.map((json) => SecurityLog.fromJson(json)).toList();
      }
    } catch (e) {
      if (kDebugMode) {
  ...
  Future<List<SecurityLog>> getUserActivities(
      DateTime startDate, DateTime endDate) async {
    // Filter logs by date range and only include user-related activities
    return _securityLogs
        .where((log) =>
            log.timestamp.isAfter(startDate) &&
            log.timestamp.isBefore(endDate) &&
            (log.type == SecurityEventType.loginSuccess ||
                log.type == SecurityEventType.loginFailed ||
                log.type == SecurityEventType.logout ||
                log.type == SecurityEventType.sessionTimeout ||
                log.type.toString().toLowerCase().contains('user')))
        .toList();
  }
  ...
  Future<bool> checkNetworkSecurity() async {
    try {
      ...
      // If we can't check connectivity, assume it's safe to operate offline
      _logSecurityEvent(
        SecurityEventType.networkCheckFailed,
        'Network security check failed, allowing offline operation: $e',
      );
