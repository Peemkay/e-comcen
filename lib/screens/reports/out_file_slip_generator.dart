...
class OutFileSlipGenerator extends StatefulWidget {
  const OutFileSlipGenerator({super.key});

  @override
  State<OutFileSlipGenerator> createState() => _OutFileSlipGeneratorState();
}

class _OutFileSlipGeneratorState extends State<OutFileSlipGenerator> {
  ...
  Future<void> _loadData() async {
    try {
      ...
      // Load outgoing dispatches for OUT FILE
      ...
      final savedSlips = [
        {
          'id': '1',
          'name':
              'OUT FILE Slip - ${DateFormat('yyyy-MM-dd').format(DateTime(2023, 1, 15))}',
        ...
        },
        {
          'id': '2',
          'name':
              'OUT FILE Slip - ${DateFormat('yyyy-MM-dd').format(DateTime(2023, 2, 20))}',
      ...
      if (mounted) {
        setState(() {
          ...
          _allDispatches = outgoingDispatches;
          _filteredDispatches = List.from(outgoingDispatches);
    ...
    setState(() {
      _filteredDispatches = _allDispatches.where((dispatch) {
        ...
        if (_selectedFromUnit != null &&
            ...
            dispatch.sentBy != _selectedFromUnit) {
          return false;
        }
        ...
        if (_selectedToUnit != null &&
            ...
            dispatch.recipientUnit != _selectedToUnit) {
          return false;
        }
  ...
  // Generate and save the OUT FILE slip as PDF
  Future<void> _generateOutFileSlip() async {
    ...
    try {
      ...
      final fileName =
          'out_file_slip_${DateFormat('yyyyMMdd').format(_selectedDate)}.pdf';

      // Get the Documents directory
      final documentsDir = await getApplicationDocumentsDirectory();

      // Create the Dispatches/OutFile directory if it doesn't exist
      final saveDir = Directory('${documentsDir.path}/Dispatches/OutFile');
      ...
      String? selectedPath = await FilePicker.platform.saveFile(
        dialogTitle: 'Save OUT FILE Slip',
        fileName: fileName,
      ...
      final newSlip = {
        ...
        'name':
            'OUT FILE Slip - ${DateFormat('yyyy-MM-dd').format(_selectedDate)}',
        'path': file.path,
        'date': _selectedDate,
        'dispatches': _filteredDispatches
            .map((dispatch) => {
                  'referenceNumber': dispatch.referenceNumber,
                  'originatorsNumber': dispatch
                      .referenceNumber, // Using reference number as originator's number
                  'priority': dispatch.priority,
                  'addrFrom': dispatch.sentBy,
                  'addrTo': dispatch.recipientUnit,
                  'thi': DateFormat('HH:mm')
                      .format(dispatch.sentDate), // Using sent date as THI
                  'tcl': '', // No TCL for outgoing dispatches
                })
            .toList(),
      };
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('OUT FILE slip saved to: ${file.path}'),
      ...
      debugPrint('Error generating OUT FILE slip: $e');
      setState(() {
        _isGenerating = false;
      });
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Error generating OUT FILE slip: $e'),
            backgroundColor: Colors.red,
          ),
        );
      }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('OUT FILE Slip Generator'),
                        ...
                        child: Padding(
                          ...
                          child: isSmallScreen
                                    ...
                                    Container(
                                      ...
                                      child: ClipRRect(
                                        ...
                                        child: SingleChildScrollView(
                                          ...
                                          child: _buildOutFileSlipPreview(),
                                        ),
                                      ),
                                    ),
                                  ],
                              ...
                              : Row(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    // Left Column - OUT FILE Slip Preview
                                    Expanded(
                                      flex: 3,
                                      child: Column(
                                        crossAxisAlignment:
      ...
      children: [
        ...
        SizedBox(
          ...
          child: ElevatedButton.icon(
            onPressed: _isGenerating ? null : _generateOutFileSlip,
            icon: _isGenerating
                ? const SizedBox(
                    width: 20,
                    height: 20,
  ...
  Widget _buildOutFileSlipPreview() {
    return RepaintBoundary(
      ...
      child: Column(
        ...
        children: [
          ...
          Center(
            child: Text(
              'OUT MESSAGE OF: ${DateFormat('dd/MM/yyyy').format(_selectedDate)}',
              style: TextStyle(
        ...
        TableRow(
          children: [
            ...
            _buildTableCellPreview(dispatch
                .referenceNumber), // Using reference number as originator's number
            _buildTableCellPreview(dispatch.sentBy), // Using sentBy as addrFrom
            _buildTableCellPreview(
                dispatch.recipientUnit), // Using recipientUnit as addrTo
            _buildTableCellPreview(DateFormat('HH:mm')
                .format(dispatch.sentDate)), // Using sentDate as THI
            _buildTableCellPreview(''), // No TCL for outgoing dispatches
          ],
        ),
      );
    }
