...
import 'in_file_form.dart';

class InFileListScreen extends StatefulWidget {
  const InFileListScreen({super.key});

  @override
  State<InFileListScreen> createState() => _InFileListScreenState();
}

class _InFileListScreenState extends State<InFileListScreen> {
  ...
  void _applyFiltersAndSort() {
    ...
    List<IncomingDispatch> filteredDispatches =
        List.from(_dispatchService.getIncomingDispatches());
      ...
      filteredDispatches = filteredDispatches.where((dispatch) {
        return dispatch.referenceNumber
            ...
            dispatch.senderUnit
                .toLowerCase()
                .contains(_searchQuery.toLowerCase()) ||
            dispatch.addrTo.toLowerCase().contains(_searchQuery.toLowerCase());
      }).toList();
    }
    ...
    if (_filterSenderUnit != 'All') {
      filteredDispatches = filteredDispatches
          .where((dispatch) => dispatch.senderUnit == _filterSenderUnit)
          .toList();
    }
    ...
    if (_filterAddrTo != 'All') {
      filteredDispatches = filteredDispatches
          .where((dispatch) => dispatch.addrTo == _filterAddrTo)
          .toList();
    }
  ...
  void _addNewDispatch() {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => const InFileForm(),
      ),
    ).then((_) => _loadDispatches());
  }
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            ...
            onPressed: () {
              Navigator.pushNamed(
                  context, AppConstants.inFileSlipGeneratorRoute);
            },
            tooltip: 'Generate IN FILE Slip',
          ),
        ],
      ),
      body: Column(
          ...
          Padding(
            ...
            child: TextField(
              decoration: InputDecoration(
                hintText: 'Search IN FILE dispatches...',
