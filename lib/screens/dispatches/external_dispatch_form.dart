...
class ExternalDispatchForm extends StatefulWidget {
  final ExternalDispatch? dispatch;

  const ExternalDispatchForm({super.key, this.dispatch});

  @override
  State<ExternalDispatchForm> createState() => _ExternalDispatchFormState();
}

class _ExternalDispatchFormState extends State<ExternalDispatchForm> {
  ...
  void _saveDispatch() {
    if (_formKey.currentState!.validate()) {
      ...
      final dispatch = ExternalDispatch(
        id: _isEditing
            ? widget.dispatch!.id
        ...
        logs: _isEditing
            ? [
                ...
                DispatchLog(
                  ...
                  performedBy: 'Admin',
                  notes: _isEditing
                      ? 'Updated external dispatch'
                      : 'Created new external dispatch',
                ),
              ]
            : [
                DispatchLog(
                  ...
                  action: 'Created',
                  performedBy: 'Admin',
                  notes: 'Created new external dispatch',
                ),
              ],
        trackingStatus:
      ...
      // Save to service
      if (_isEditing) {
        _dispatchService.updateExternalDispatch(dispatch);
      } else {
        _dispatchService.addExternalDispatch(dispatch);
      }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(
            _isEditing ? 'Edit External Dispatch' : 'New External Dispatch'),
            ...
            children: [
              ...
              TextFormField(
                ...
                decoration: const InputDecoration(
                  ...
                  border: OutlineInputBorder(),
                  prefixIcon: Icon(FontAwesomeIcons.addressBook, size: 16),
                ),
                validator: (value) {
                  if (value == null || value.isEmpty) {
